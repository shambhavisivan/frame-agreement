	...
	describe("actions", () => {
		it("should create an action using function that returns dispatch", () => {
			...
			actions.registerMethod(
				expectedAction.payload.name,
				expectedAction.payload.method
			)(store.dispatch);
		...
		it("should call actions for creating custom rule group without error", () => {
			return actions
				...
				.then((resp) => {
					...
					expect(executed_actions[0]).toEqual({
						...actions._loadAccounts(),
						payload: { data: resp },
					});
		...
		it("should call getApprovalHistory without an error", () => {
			return actions
				...
				.then((resp) => {
					const executed_actions = store.getActions();
					expect(executed_actions[0].type).toEqual(
		...
		it("should create an action setCustomData using function that returns dispatch", () => {
			...
			actions.setCustomData(
				expectedAction.payload.faId,
		...
		it("should create negotiate actions using dispatch methods", () => {
			...
			expect(store.getActions()).toEqual([
				expectedAction_negotiate,
				expectedAction_apiNegotiate,
				expectedAction_bulkNegotiate,
			]);
		});
		...
		it("should call createNewVersionOfFrameAgrement without an error", () => {
			return actions
				.createNewVersionOfFrameAgrement("a1t1t0000009wpQAAQ")(
					store.dispatch
				)
				.then((resp) => {
					const executed_actions = store.getActions();
					...
					expect(
						resp.csconta__Agreement_Name__c.endsWith("_v2")
					).toBeTruthy();
				});
		});
		...
		it("should call submitForApproval without an error", () => {
			return expect(
				actions.submitForApproval("recordId")
			).resolves.toBeTruthy();
		});
		...
		it("should create an action toggleFieldVisibility using function that returns dispatch", () => {
			...
			store.dispatch(
				actions.toggleFieldVisibility(expectedAction.payload)
			);
			expect(store.getActions()).toEqual([expectedAction]);
		});
		...
		it("should create an action toggleFaFieldVisibility using function that returns dispatch", () => {
			...
			store.dispatch(
				actions.toggleFaFieldVisibility(expectedAction.payload)
			);
		...
		it("should create an action out of several pure dispatch methods", () => {
			...
			expect(store.getActions().map((disp) => disp.type)).toEqual(
				expectedActionOrder
			);
		});
		...
		it("should create an cloneFrameAgreement action", () => {
			return actions
				...
				.then((resp) => {
					...
					expect(resp.csconta__Agreement_Name__c).toEqual(
						"Frame Agreement - Test #1"
					);
				});
		});
		...
		it("should create an addFaToMaster action", () => {
			return actions
				.addFaToMaster("a1t1t0000009wpQAAQ", ["a1t1t000000A0gJAAS"])(
					store.dispatch
				)
				.then((resp) => {
					const executed_actions = store.getActions();
		...
		it("should create an removeFaFromMaster action", () => {
			return actions
				.removeFaFromMaster("a1t1t0000009wpQAAQ", [
					"a1t1t000000A0gJAAS",
				])(store.dispatch)
				.then((resp) => {
					const executed_actions = store.getActions();
		...
		it("should create an addProductsToFa action", () => {
			return actions
				.addProductsToFa("a1t1t0000009wpQAAQ", ["a1F1t0000001JBjEAM"])(
					store.dispatch
				)
				.then((resp) => {
					const executed_actions = store.getActions();
		...
		it("should create an removeProductsFromFa action", () => {
			return actions
				.removeProductsFromFa("a1t1t0000009wpQAAQ", [
					"a1F1t0000001JBjEAM",
				])(store.dispatch)
				.then((resp) => {
					const executed_actions = store.getActions();
