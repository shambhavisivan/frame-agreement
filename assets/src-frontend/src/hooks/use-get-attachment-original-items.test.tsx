...
import { useGetAttachmentOriginalItems } from './use-get-attachment-original-items';
...
describe('test useGetAttachmentOriginalItems', () => {
	...
	const createAttExtendedSpy = jest
		...
		.mockReturnValue(mockAttachmentOriginalItems);

	test('should call the useGetattachmentOriginalItems hook and return extended attachment data.', async () => {
		const { result } = renderHook(() =>
			useGetAttachmentOriginalItems(attachment, QueryStatus.Success)
		);
		...
		expect(result.current).toEqual(mockAttachmentOriginalItems);
	});
});
