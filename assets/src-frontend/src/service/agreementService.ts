...
class AgreementService {
	...
	public getFaAttachment = async (faId: string): Promise<Attachment> => {
		...
		this._detailsPageProvider.dispatch({
			...
			payload: {
				attachment,
				attachmentOriginalItems: (this._getAttExtended(
					attachment
	...
	private _getAttExtended = async (attachment: Attachment): Promise<AttachmentOriginalItems> => {
		...
		if (cachedCps) {
			...
			commercialProducts = await remoteActions.queryProducts(productIds, null, null, 0, []);
			this._queryCache.setQueryData(
				['commercialProducts', productIds, null, null, 0, []],
				commercialProducts
			);
		}
		...
		if (cachedCpData) {
			...
			productsData = await remoteActions.getCommercialProductData(productIds);
			this._queryCache.setQueryData(['commercialProductData', productIds], productsData);
		}

		if (cachedAddons) {
			standaloneAddons = cachedAddons;
		} else {
			standaloneAddons = await remoteActions.getStandaloneAddons();
			this._queryCache.setQueryData([QueryKeys.standaloneAddons], standaloneAddons);
		}

		const attachmentOriginalItems: AttachmentOriginalItems = createAttExtended(
			commercialProducts,
			productsData,
			standaloneAddons
		);
