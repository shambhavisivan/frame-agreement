	...
	constructor(props) {
		...
		try {
			...
			let startingIndex = this.props.frameAgreements[
				this.props.faId
			]._ui.approval.listProcess[0].StepsAndWorkitems.findIndex(
				(step) => {
			...
			this.isInitiator =
				...
				this.props.frameAgreements[this.props.faId]._ui.approval
					.listProcess[0].StepsAndWorkitems[startingIndex]
					.OriginalActorId ===
					this.props.frameAgreements[this.props.faId]._ui.approval
						.currentUser;
		} catch (err) {}
				...
				.then((response) => {
					if (response[0]) {
						this.props.createToast(
							...
							actionType +
								" " +
								window.SF.labels.toast_approvalAction_success
						);
					} else {
						this.props.createToast(
							"error",
							...
							actionType +
								" " +
								window.SF.labels.toast_approvalAction_failed
						);
					}
				})
				.then(() => {
					...
					this.props.executeFrameAgreementAction(
						this.props.faId,
						frameAgreementActions.CLONE
					);
				})
				.finally(() => {
					this.props.toggleFrameAgreementOperations(false);
						...
						<div className="approval-table-list-container">
							<div className="table-list-header">
								...
								<div
									...
									title={
										window.SF.labels
											.approval_table_header_assignedTo
									}
								>
									<span>
										{
								...
								<div
									...
									title={
										window.SF.labels
											.approval_table_header_actualApprover
									}
								>
									<span>
										{
								...
								<div
									...
									title={
										window.SF.labels
											.approval_table_header_comments
									}
								>
									<span>
										{
