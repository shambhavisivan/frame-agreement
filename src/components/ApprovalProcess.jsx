...
import { approveRejectRecallRecord, reassignApproval, createToast } from "../actions";
...
class ApprovalProcess extends React.Component {
	constructor(props) {
		...
		try {
			...
			let startingIndex = this.props.approval.listProcess[0].StepsAndWorkitems.findIndex(step => {
				return step.StepStatus === "Started";
			})
			this.isInitiator = this.isInitiator || this.props.approval.listProcess[0].StepsAndWorkitems[startingIndex].OriginalActorId === this.props.approval.currentUser
	...
	componentWillUpdate(newProps) {
		...
		try {
			...
			newActionRequired = newProps.approval.listProcess[0].StepsAndWorkitems[0].IsPending;

			let startingIndex = newProps.approval.listProcess[0].StepsAndWorkitems.findIndex(step => {
				return step.StepStatus === "Started";
			})

			newIsInitiator = newIsInitiator || newProps.approval.listProcess[0].StepsAndWorkitems[startingIndex].OriginalActorId === newProps.approval.currentUser
	...
	approvalAction(actionType, reassignTo = null) {
		...
		if (actionType === "Reassign") {
			this.props.reassignApproval(this.props.faId, reassignTo || this.props.approval.currentUser)
		} else {
			...
			<div className="approval-history approval-table-list">
				...
				<div className="approval-table-list-header">
					...
					<div className="fa-flex fa-flex-middle" onClick={() => {this.refreshApprovalHistory()}}>
						<Icon svg-class={"approval-refresh" + (this.state.loading ? ' rotating' : '')} name="refresh" width="14" height="14" color="#747474" />
					</div>
				...
				{this.state.open && (<div className="approval-table-list-container">
					...
					{this.props.approval.listProcess.map((process, i) => {
						return (<ul key={process.Id} className="table-list">
						{process.StepsAndWorkitems.map((step, i) => {
							return (
								<li key={'approvalStep'+i} className="list-row">
									<div className="list-cell">{(step.ProcessNode && step.ProcessNode.Name) || 'Approval Request Submitted '}</div>
									<div className="list-cell">{moment(step.CreatedDate).format('MM/D/YYYY, HH:mm')}</div>
									<div className="list-cell"><span className={"status " + step.StepStatus}>{step.StepStatus}</span></div>
									<div className="list-cell">{step.Actor && step.Actor.Name}</div>
									<div className="list-cell">{step.OriginalActor && step.OriginalActor.Name}</div>
									<div className="list-cell">{step.Comments || "-/-"}</div>
								</li>
							);
						})}
						</ul>)
