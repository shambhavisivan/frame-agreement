...
class FrameAgreementRow extends React.Component {
	constructor(props) {
		super(props);
		this.statusClass = "fa-chip " + (this.props.agreement.csconta__Status__c === "Draft" ? "" : "fa-chip-dark");
		...
		return (
			<div className="fa-panel">
				<div className="fa-icon fa-icon-group" onClick={this.showMenu}>
					<button className="fa-button-bare" aria-label="Dropdown button" aria-expanded={this.state.menu}>
						<Icon name="threedots_vertical" width="16" height="16" color="#0070d2" />
					</button>
					{this.state.menu && (
						<div className="fa-dropdown" aria-hidden={!this.state.menu} ref={this.setWrapperRef}>
							<button className="fa-dropdown-button" onClick={() => this.menuAction("edit")}>
								<Icon name="edit" height="14" width="14" color="#0070d2" />
								{window.SF.labels.faMenuActionEdit}
							</button>
							<button className="fa-dropdown-button" onClick={() => this.menuAction("clone")}>
								<Icon name="copy" height="14" width="14" color="#0070d2" />
								{window.SF.labels.faMenuActionClone}
							</button>
							<button className="fa-dropdown-button" onClick={() => this.menuAction("delete")}>
					...
					<Link className="fa-panel-body" to={`/agreement/${this.props.agreement.Id}`}>
