...
class RcmBulk extends Component {
    ...
    componentWillMount() {
        let rateCardLines = Object.values(this.props.rcm_data).reduce((acc, cur) => {
    ...
    onPropertyChange(e) {
      var defaultProperty = this.state.propertyData[e.target.value] ? this.state.propertyData[e.target.value][0] : '';
      this.setState({ selectedProperty: (e.target.value || null), selectedPropertyValue: defaultProperty })
    }
    ...
    onCheckboxToggle(rclId) {
      this.setState({selection: { ...this.state.selection, [rclId]: !this.state.selection[rclId]}}, () => {
    ...
    applyFilter() {
        ...
        rateCardLines = rateCardLines.filter(rcl => {
          var _targeted = (rcl[this.state.selectedProperty] && rcl[this.state.selectedProperty] === this.state.selectedPropertyValue);
          _stateSelection[rcl.Id] = _targeted;
          return (rcl[this.state.selectedProperty] && rcl[this.state.selectedProperty] === this.state.selectedPropertyValue)
        });


        this.setState({rateCardLines: rateCardLines, selection: _stateSelection}, () => {
          this.checkSelectAllState();
        });
    }
    ...
    resetFilter() {
        ...
        this.setState({rateCardLines: rateCardLines, selectedProperty: '', selection: _stateSelection}, () => {
          this.checkSelectAllState();
        });
    }
        ...
        return (
            <div> 
              ...
              <div className="bulk-filter-container">
                  <h4>Filter rate card lines  <span>({this.state.rateCardLines.length})</span></h4>

                  <div className="bulk-filter-section">
                    <label>Select rate card line property:</label>

                    <select className="rcm-input_sm" value={this.state.selectedProperty || ''} onChange={this.onPropertyChange}>
                  ...
                  <label>Select value:</label>
                   ...
                   onChange={this.onPropertyValueChange}>
                    {this.state.propertyData[this.state.selectedProperty] && this.state.propertyData[this.state.selectedProperty].map(val => {
                         return (
                ...
                <div className="bulk-button-container">
                  <button className="slds-button rcm-input_sm slds-button--neutral" disabled={this.state.propertyData[this.state.selectedProperty] ? false : true} onClick={this.applyFilter}>Apply</button>
                  <button className="slds-button rcm-input_sm slds-button--neutral" onClick={this.resetFilter}>Reset</button>
                </div>
              </div>
              ...
              <div>
                <table className="slds-table slds-table_bordered slds-no-row-hover">
                  <thead>
                    <tr>
                      <th><div className="slds-truncate"><span className={"slds-checkbox_faux " + (this.state.allToggle ? 'checked' : '')} onClick={this.onRateCardLineSelectAll}></span></div></th>
                      <th><div className="slds-truncate">Name</div></th>
                      <th><div className="slds-truncate">Unit</div></th>
                      {
                        Object.keys(this.props.custom_fields).filter(cf => this.props.custom_fields[cf]).map(cf => {
                          return (<th key={'th-'+cf}><div className="slds-truncate">{cf}</div></th>)
                        })
                      }
                      <th><div className="slds-truncate">Original Value</div></th>
                      <th><div className="slds-truncate">Negotiated Value</div></th>
                    </tr>
                  </thead>
                  <tbody>
                    {this.state.rateCardLines.map(rcl => {
                      return (
                        <tr key={rcl.Id}>
                          <td><span className={"slds-checkbox_faux " + (this.state.selection[rcl.Id] ? 'checked' : '')} onClick={() => this.onCheckboxToggle(rcl.Id)}></span></td>
                          <td>{rcl.Name}</td>
                          <td>{rcl.Unit}</td>
                          {
                            Object.keys(this.props.custom_fields).filter(cf => this.props.custom_fields[cf]).map(cf => {
              ...
              <div className="bulk-discount-container">
                ...
                <div className="bulk-discount-section">
                    ...
                    <input className="rcm-input_sm form-control" type="number" value={this.state.discount} onChange={(e) => this.setState({discount: +e.target.value})}/>

                </div>
                <div className="bulk-discount-section">
                <label>&nbsp;</label> 
                    <button disabled={this.state.rateCardLines.length ? false : true}  style={{display: 'block'}} className="slds-button rcm-input_sm slds-button--brand" id="save-rcm-modal" onClick={this.applyDiscount}>
                      Apply change {this.state.discount !== 0 && <span>{`(${this.state.discount > 0 ? '+' : ''}${this.state.discount}${this.state.discountMode === "fixed" ? '' : '%'})`}</span>}
                    </button>
                </div>
