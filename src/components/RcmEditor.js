        ...
        {Object.values(this.props.group_data).map(group => {
          return (
            ...
            <div className="ed-group" key={group.GroupName}>
              ...
              <div className={"ed-group-header" + (group.validity ? '' : ' invalid')} onClick={() => this.toggleCollapse(group.GroupName)}>
                  <h3>{ group.GroupName !== 'undefined' && ("Group: " + group.GroupName || 'N/A')}
                    {group.Min !== null && <span className="q-restriction">&darr;{group.Min}</span>}
                    {group.Max !== null && <span className="q-restriction">&uarr;{group.Max}</span>}
                    <span className={"collapse-chevron chevron-" + (this.state.group_collapse[group.GroupName] ? 'left' : 'bottom')}></span>
                  </h3>
              </div>

              <Collapse className="ed-group-body" isOpened={!this.state.group_collapse[group.GroupName]}>
                  ...
                  <tbody>
                    {group.rateCardIds.map(rcId => {
                      ...
                      return (
                        ...
                        <tr className={"ed-group-rc" + (_isDefault ? 'disabled' : '')} key={rcId} onClick={() => !_isDefault && this.eligableForAdding(this.props.rcm_data[rcId]) && this.props.toggleRateCard(this.props.rcm_data[rcId])}>
                          ...
                          <td>
                            <div className="slds-truncate">
                              {(() => {
                                  switch (_isDefault) {
                                      case false:
                                          return <span className={"slds-checkbox_faux " + (this.props.rcm_data[rcId].added ? 'checked' : '')}></span>
                                      case true:
                                          return <span className="slds-checkbox_faux checked disabled"></span>
                                  }
                              })()}
                            </div>
                          </td>
                          ...
                          <td>{(() => {
                             ...
                             } else return <span className="close-icon red"></span>
                          })()}</td>
                          <td>{this.props.rcm_data[rcId].rateCardLines.length}</td>
                        </tr>
