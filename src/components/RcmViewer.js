...
class RcmViewer extends Component {
	constructor(props) {
		...
		this.onCustomRateCardLineChange = this.onCustomRateCardLineChange.bind(
			this
		);
	...
	onNegotiatedValueChanged(value, rcId, rclId) {
		this.props.rcm_data[rcId].rateCardLines = this.props.rcm_data[
			rcId
		].rateCardLines.map(rcl => {
			if (rcl.Id === rclId) {
	...
	onCustomRateCardLineChange(rcl) {
		if (this.state.newRclObj[rcl.Id] !== undefined) {
			this.setState({
				newRclObj: { ...this.state.newRclObj, [rcl.Id]: false }
			});
		}
	...
	addCustomRcl(rcId) {
		...
		this.setState({
			newRclObj: { ...this.state.newRclObj, [newRcl.Id]: true }
		});
		...
		return (
			<div id="rcmViewer">
				<ul
					id="vw-cardList"
					className={
						'' + (this.props.settings.AllowAddingRCL ? '' : 'disabled-adding')
					}
				>
					{Object.values(this.props.rcm_data)
						.filter(rc => rc.added || rc.IsDefault)
						.sort(this.sortByDefault)
						.map(rc => {
							return (
								<li key={rc.Id}>
									<span
										className={'rc-name ' + (rc.IsDefault ? 'default' : '')}
									>
										{rc.Name}
									</span>
									...
									<table className="vw-rcTable slds-table slds-table_bordered slds-no-row-hover">
										...
										<tbody>
												...
												return (
													<tr key={rcl.Id}>
														...
														{Object.keys(this.props.custom_fields)
															...
															.map((cf, i) => {
																return (
																	<td key={'th-' + cf + i}>{rcl[cf] || ''}</td>
																);
															})}
														<td>{rcl.originalValue}</td>
														<td>
															<RclInput
																onChange={this.onNegotiatedValueChanged}
																rcid={rc.Id}
																rclid={rcl.Id}
																value={rcl.negotiatedValue}
															/>
														</td>
											...
											{rc.customRateCardLines &&
												rc.customRateCardLines.map(rcl => {
													return (
														<CustomRcl
															editing={this.state.newRclObj[rcl.Id]}
															key={rcl.Id}
															rcl={rcl}
															customFields={this.props.custom_fields}
															onChange={this.onCustomRateCardLineChange}
															onRemove={this.removeCustomRcl}
														/>
													);
												})}
										</tbody>
									</table>
									{this.props.settings.AllowAddingRCL && (
										<div className="vw-add-custom">
											<a
												href="javascript:void(0)"
												onClick={() => this.addCustomRcl(rc.Id)}
											>
												Add custom rate card line
											</a>
										</div>
									)}
								</li>
					...
					{!Object.values(this.props.rcm_data).filter(
						rc => rc.added || rc.IsDefault
					).length && <li> No rate cards</li>}
				</ul>
			</div>
		);
	}
