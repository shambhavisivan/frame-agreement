...
class NegotiationModal extends Component {
	constructor(props) {
		...
		this._addons = this.commercialProducts.reduce(
			(acc, val) =>
				acc.concat(
		...
		console.log('***********************************');
		console.warn(this.props.attachment);
		console.log('***********************************');
	...
	componentWillMount() {
		...
		let mergedChargesAttachment = Object.values(this.props.attachment).reduce(
			(acc, val) => acc.concat(val._charges || []),
			[]
		)[0];
		// let charges = validateCharges(this._charges, mergedChargesAttachment);
		let charges = {};
	...
	applyDiscount() {
		...
		for (var key in selected.addons) {
			selected.addons[key].negotiatedValue = applyDiscountRate(
				selected.addons[key][ADDON_VALUE_FIELD],
				this.state
			);
		}
		...
		for (var key in selected.rated) {
			selected.rated[key].negotiatedValue = applyDiscountRate(
				selected.rated[key][RATE_VALUE_FIELD],
				this.state
			);
		}
	...
	render() {
				...
				<ul className="table-list">
					{this.filteringProcess(this._addons).map((add, i) => {
						return (
							<li
								onClick={() => {
									this.onSelectRow(add, 'addons');
								}}
								key={add.product + '-' + add.Id}
								className={
									'list-row' +
									(this.state.selected.addons[add.Id] ? ' selected-row' : '')
								}
							>
								<div className="list-cell">
									<Checkbox readOnly={this.state.selected.addons[add.Id]} />{' '}
									{add.Name}
								</div>
								<div className="list-cell">
									{' '}
									{this._addCpMap[add.Id].length +
										'/' +
										this.commercialProducts.length}
								</div>
								<div className="list-cell">
									{add.cspmb__Recurring_Charge__c}
									{this.state.selected.addons[add.Id] &&
									this.state.selected.addons[add.Id].negotiatedValue ? (
										<span>
											/{this.state.selected.addons[add.Id].negotiatedValue}
										</span>
									) : (
		...
		tab.charges = (
			<div className="table-container">
				...
				<ul className="table-list">
					{this.filteringProcess(this._charges).map((charge, i) => {
						let negotiatedValue = null;
						try {
							charge[charge._type] = charge[charge._type];
							negotiatedValue = this.props.attachment[charge.product]._charges[
								charge.Id
							][charge._type];
						} catch (err) {}

						// let originalSpan = negotiatedValue === null ? (<span>{charge[charge._type]}</span>) : (<span className="charge-negotiated">{negotiatedValue}</span>);
						// let negotiatedSpan = negotiatedValue === null ? "" : (<span> ({charge[charge._type]})</span>);
						...
						return (
							...
							>
								<div className="list-cell">
									<Checkbox readOnly={this.state.selected.charges[charge.Id]} />{' '}
									{charge.Name}
								</div>
								<div className="list-cell">{charge.chargeType}</div>
								<div className="list-cell">
									{(negotiatedValue || charge[charge._type]).toFixed(2)}
								</div>
								<div className="list-cell">
			...
			>
				...
				<div className="modal-body">
					...
					<div className="filter-container">
						{this.state.tab === 'charges' && (
							<div>
								<div className="label-text">Unique Rows</div>
								<Toggle
									onChange={val => {
										this.setState({
											filter: { ...this.state.filter, unique: val }
										});
									}}
				...
				<div className="modal-footer">
					<button
						className="slds-button slds-button--neutral"
						onClick={() => {
							this.props.applyDiscountToFrameAgreement(this.state.selected);
						}}
					>
