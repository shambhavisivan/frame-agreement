...
import { validateAddons, validateCharges, validateRateCardLines } from "../negotiation/Validation";
...
class NegotiationModal extends Component {
  constructor(props) {
    ...
    this.grouped_addons = this._addon_assoc.reduce(function(acc, level) {
      (acc[level["cspmb__Add_On_Price_Item__c"]] = acc[level["cspmb__Add_On_Price_Item__c"]] || []).push(level);
      return acc;
    }, {});
    /*********************************************************************************************************************************************************/
  ...
  componentWillMount() {
    ...
    this.commercialProducts.forEach(cp => {
      if (cp._charges) {
        charges = {
          ...charges,
          ...validateCharges(cp._charges, cp.cspmb__Authorization_Level__c, this.props.attachment[cp.Id]._charges)
        };
      }
    });
  ...
  applyDiscount() {
    ...
    this.setState({ attachment }, () => {
      this.props.createToast("info", window.SF.labels.toast_discount_calculated_title, window.SF.labels.toast_discount_calculated);
          ...
          {Object.keys(this.grouped_addons).map(add => {
            ...
            return (
              <li
                ...
                key={add}
                className={"list-row" + (this.state.selected.addons[add] ? " selected-row" : "")}
              >
                <div className="list-cell">
                  <Checkbox readOnly={this.state.selected.addons[add]} /> {addons_name}
                </div>
                <div className="list-cell"> {addons_size + "/" + this.commercialProducts.length}</div>
                <div className="list-cell"> {this.grouped_addons[add][0].cspmb__One_Off_Charge__c || "N/A"}</div>
                <div className="list-cell"> {this.grouped_addons[add][0].cspmb__Recurring_Charge__c || "N/A"}</div>
              </li>
            );
          })}
        </ul>
            ...
            .map((charge, i) => {
              return (
                <li
                  ...
                  className={"list-row" + (this.state.selected.charges[charge.Id] ? " selected-row" : "")}
                >
                  <div className="list-cell">
                    <Checkbox readOnly={this.state.selected.charges[charge.Id]} /> {charge.Name}
                  </div>
                  <div className="list-cell"> {this._chCpMap[charge.Id].length + "/" + this.commercialProducts.length}</div>
                  <div className="list-cell">{charge.chargeType}</div>
                  <div className="list-cell">{charge[charge._type].toFixed(2)}</div>
                </li>
              );
            })}
        </ul>
                ...
                <li key={rc.product + "-" + rc.Id} className="list-item selectable">
                  <div className="rc-title">
                    ...
                    <div className="title-content">
                      ...
                      {rc.Name}
                      <span className="product-count">{this._rcCpMap[rc.Id].length}</span>
                    </div>
                      ...
                      .map((rcl, i) => {
                        return (
                          <li
                            ...
                            key={rcl.Id}
                            className={"list-row" + (this.state.selected.rated[rcl.Id] ? " selected-row" : "")}
                          >
                            <div className="list-cell">
                            ...
                            <div className="list-cell">
                              ...
                              {this.state.selected.rated[rcl.Id] && this.state.selected.rated[rcl.Id].negotiatedValue ? (
      ...
      >
        ...
        <div className="modal-body fa-modal-body">
          <div className="products-container">
            <div className="label-text">{window.SF.labels.modal_bulk_selected_title}</div>
            <ul>
          ...
          <div className="action-container">
            <div className="box">
              <div className="button-group toggle-buttons">
                <button
                  className={"fa-margin-right-sm fa-button button--" + (this.state.tab === "addons" ? "brand" : "neutral")}
                  onClick={() => {
                    this.setTab("addons");
                  }}
                >
                  {window.SF.labels.products_addons} {this.state.count.addons ? <span>({this.state.count.addons})</span> : ""}
                </button>
                <button
                  className={"fa-margin-right-sm fa-button button--" + (this.state.tab === "charges" ? "brand" : "neutral")}
                  onClick={() => {
                    this.setTab("charges");
                  }}
                >
                  {window.SF.labels.products_charges} {this.state.count.charges ? <span>({this.state.count.charges})</span> : ""}
                </button>
                <button
                  className={"fa-button button--" + (this.state.tab === "rated" ? "brand" : "neutral")}
                  onClick={() => {
                    this.setTab("rated");
                  }}
                >
                  {window.SF.labels.products_rates} {this.state.count.rated ? <span>({this.state.count.rated})</span> : ""}
                </button>
              </div>
            </div>
            <div className="fa-margin-top-sm">
              <div className="fa-margin-bottom-sm">
                ...
                <div className="button-group toggle-buttons">
                  <button
                    className={"fa-margin-right-sm fa-button button--" + (this.state.discountMode === "percentage" ? "brand" : "neutral")}
                    onClick={() => {
                  ...
                  <button
                    className={"fa-button button--" + (this.state.discountMode === "fixed" ? "brand" : "neutral")}
                    onClick={() => {
              ...
              <div>
                ...
                />
                <button disabled={!this.state.countTotal} className="fa-button button--neutral" onClick={this.applyDiscount}>
