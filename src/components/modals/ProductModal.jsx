...
class ProductModal extends Component {
  constructor(props) {
    ...
    */
    ...
    this.notAddedCommercialProducts = this.props.commercialProducts.filter(cp => !this.addedProductsIds.includes(cp.Id));
  ...
  async applyFilter() {
    ...
    this.setState({
      commercialProducts: result.filter(cp => !this.addedProductsIds.includes(cp.Id))
    });
  }
  ...
  render() {
    return (
      ...
      >
        ...
        <div className={"modal-body fa-modal-body " + (this.state.panel ? "panel-open" : "panel-closed")}>
          <div className="modal-panel">
              ...
              {Object.keys(this.state.filter).map(key => {
                ...
                return (
                  <div className="filter-category" key={key}>
                    ...
                    >
                      <Icon name={category.open ? "chevrondown" : "chevronright"} width="12" height="12" color="#747474" />
            ...
            <div className="panel-filter-buttons fa-margin-bottom-sm">
              <button onClick={this.resetFilter} className="fa-button fa-margin-right-xsm" disabled={false}>
                {window.SF.labels.modal_categorization_btn_clear}
              </button>
              <button onClick={this.applyFilter} className="fa-button" disabled={false}>
          ...
          <div className="modal-table-container">
            <div className="modal-navigation">
              {this.props.settings.CategorizationData.length && (
                <div className="categorization-container" onClick={this.togglePanel}>
                  <Icon name="color_swatch" width="14" height="14" color="#0070d2" />
                  <div className="categorization-switch">{window.SF.labels.modal_categorization_switch}</div>
                </div>
              )}
            ...
            <div className="modal-product-list">
              <div className="product-list-header">
                <div className="header-th">{window.SF.labels.products_productNameHeaderCell}</div>
                {this.priceItemFields.map(f => {
                  return (
                    <div key={f.name} className="header-th">
                      <span>{this.props.settings.FACSettings.truncate_product_fields ? truncateCPField(f.name) : f.name}</span>
                    </div>
                  );
                })}
              </div>
                  ...
                  .map(cp => {
                    return (
                      ...
                      >
                        <span>{cp.Name}</span>
                        {this.priceItemFields.map(f => {
                          return <span key={cp.Id + "-" + f.name}>{cp[f.name] || "-"}</span>;
                        })}
                      </div>
                    );
                  })}
        ...
        <div className="modal-footer">
          <button onClick={this.addProducts} className="slds-button slds-button--brand" disabled={!this.state.actionTaken}>
