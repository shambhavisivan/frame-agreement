...
export class CommercialProduct extends React.Component {
	...
	async onNegotiate(type, data, negotiationContext) {
		let initialFrameAgreementProducts =
			this.props.currentFrameAgreement._ui.attachment?.products ?? {};
		...
		if (type === "_product") {
			this.props.setValidation(
				...
				validateProduct(
					...
					{
						negotiatedOneOff:
							initialFrameAgreementProducts[this.productId]
								?._product?.oneOff,
						negotiatedRecurring:
							initialFrameAgreementProducts[this.productId]
								?._product?.recurring,
	...
	buildEventData(type, negotiationContext) {
		...
		switch (type) {
			case "_addons":
				eventHookData[this.productId].addons = {
					...negotiationContext,
				};
				return eventHookData;
			case "_charges":
				eventHookData[this.productId].charges = {
					...negotiationContext,
				};
				return eventHookData;
			case "_rateCards":
				eventHookData[this.productId].rateCards = {
					...negotiationContext,
				};
				return eventHookData;
			case "_product":
				eventHookData[this.productId].product = {
					...negotiationContext,
				};
				return eventHookData;
			case "_volume":
				eventHookData[this.productId].volume = {
					...negotiationContext,
				};
				return eventHookData;
		}
	...
	render() {
		let _attachment = this.props.frameAgreements[this.props.faId]._ui
			.attachment.products[this.productId];
		...
		const shouldDisplayRateCards = !!this.props.product?._rateCards?.reduce(
				...
				>
					...
					{this.state.open && (
						<div className="commercial-product-body">
							...
							<Tabs
								initial={
									this.props.product._addons.length ? 0 : 1
								}
							>
								<Tab
									label={window.SF.labels.products_addons}
									disabled={
										!this.props.product._addons.length
									}
								>
									<Addons
										readOnly={
								...
								<Tab
									...
									disabled={
										!this.props.product._allowances.length
									}
								>
									<Allowances
										data={this.props.product._allowances}
									/>
								</Tab>
							</Tabs>
						</div>
