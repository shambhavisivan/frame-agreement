...
export class Offer extends React.Component {
	...
	async onNegotiate(type, data, negotiationContext) {
		let initialFrameAgreementOffers =
			this.props.currentFrameAgreement._ui.attachment?.offers ?? {};
		...
		if (type === "_addons") {
			this.props.setValidation(
				...
				validateAddons(
					...
					{
						frameAgreementStatus: this.props.frameAgreements[
							this.props.faId
						].csconta__Status__c,
						facApprovedStatus: this.props.settings.FACSettings
		...
		if (type === "_product") {
			this.props.setValidation(
				...
				validateProduct(
					...
					{
						negotiatedOneOff:
							initialFrameAgreementOffers[this.offerId]?._product
								?.oneOff,
						negotiatedRecurring:
							initialFrameAgreementOffers[this.offerId]?._product
								?.recurring,
	...
	buildEventData(type, negotiationContext) {
		...
		switch (type) {
			...
			case "_charges":
				...
				return eventHookData;
			case "_rateCards":
				eventHookData[this.offerId].rateCards = {
					...negotiationContext,
				};
				return eventHookData;
			case "_product":
	...
	render() {
		let _attachment = this.props.frameAgreements[this.props.faId]._ui
			.attachment.offers[this.offerId];
		...
		let _disableLevels = !!this.props.frameAgreements[this.props.faId]._ui
			.disableDiscountLevels;
		...
		const shouldDisplayRateCards = !!this.props.offer?._rateCards?.reduce(
			(dispRateCard, rateCard) =>
				dispRateCard | !!rateCard?.rateCardLines?.length,
			false
		);
			...
			<SettingsContext.Provider value={this.props.settings}>
				<div
					className={
						...
						(this.props.validationOffers[this.offerId]
							? " invalid-product"
							: "")
					}
				>
					...
					{this.state.open && (
						<div className="commercial-product-body">
							...
							<Tabs
								initial={
									this.props.offer._addons.length ? 0 : 1
								}
							>
								<Tab
								...
								>
									<Addons
										...
										disableInputs={
											_productIgnored || _disableInputs
										}
										disableLevels={
											_productIgnored || _disableLevels
										}
										validation={
											this.props.validationOffersInfo[
												this.offerId
											].addons
										}
								...
								>
									{this.props.offer._charges.length ? (
										<Charges
											...
											levels={
												this.props.offer._discountLvIds
											}
											validation={
												this.props.validationOffersInfo[
													this.offerId
								...
								<Tab
									...
									disabled={!shouldDisplayRateCards}
								>
									<Rates
										readOnly={
											this.props
								...
								<Tab
									...
									disabled={
										!this.props.offer._allowances.length
									}
								>
									<Allowances
										data={this.props.offer._allowances}
									/>
								</Tab>
								<Tab
									label={
										window.SF.labels.offer_metadata_header
									}
									disabled={!this.props.offer._metadata}
								>
									<OffersMetadata
										data={
											this.props.offer._metadata
												.attributeMetadata
										}
									/>
								</Tab>
