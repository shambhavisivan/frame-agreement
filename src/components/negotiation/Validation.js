			...
			validation[negotiationFormat.addon.cspmb__Authorization_Level__c].forEach(
				thresh => {
					...
					if (!thresh.hasOwnProperty('cspmb__Discount_Threshold__c')) {
						...
						if (
							typeof negotiationFormat.negotiatedOneOff !== 'undefined' &&
						...
						if (
							typeof negotiationFormat.negotiatedRecurring !== 'undefined' &&
	...
	validation[data.authLevel] &&
		validation[data.authLevel].forEach(thresh => {
			...
			if (!thresh.hasOwnProperty('cspmb__Discount_Threshold__c')) {
				...
				if (
					data.negotiatedOneOff !== undefined &&
				...
				if (
					data.negotiatedRecurring !== undefined &&
	...
	if (rcArr) {
		rcArr.forEach(_rc => {
			_rc.rateCardLines.forEach(_rcl => {
				...
				if (typeof negotiationFormat.negotiatedValue !== 'undefined') {
					detailedMap = {
						...detailedMap,
