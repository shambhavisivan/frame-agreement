...
import { truncateCPField, getFieldLabel, openSFLink } from '~/src/utils/shared-service.js';
...
class RelatedLists extends Component {
	...
	render() {
		let _falseIcon = <Icon name="clear" height="14" width="14" color="#d9675d" />;
		let _trueIcon = <Icon name="success" height="14" width="14" color="#4bca81" />;
					...
					{this.props.list.columns.map(f => {
						return (
							<div key={f} className="header-th">
								<span>{getFieldLabel(this.props.list.object, f) || truncateCPField(f, true)}</span>
							</div>
						);
					})}
				</div>
				<div className="fa-modal-product-list">
					{this.props.list.records.paginate(this.state.page, this.state.pageSize).map((rlf, i) => {
						return (
							<div key={this.props.list.label + i} className="product-row">
								{this.props.list.columns.map(f => {
									...
									if (f.toLowerCase() === 'id') {
										return (
											<span className="fa-link" key={f + '-' + i} onClick={() => openSFLink(_val)}>
												{_val}
											</span>
										);
									}
					...
					{this.props.list.records.length ? null : (
						<span className="empty-rl-message">
							{window.SF.labels.rl_emptyList || 'No records for this related list'}
						</span>
					)}
				</div>
